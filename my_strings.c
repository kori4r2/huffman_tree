...
char *my_gets(FILE *stream, int str_size){
	...
	// Caso atinja o fim do arquivo nessa busca, sai da funcao com mensagem de erro e retornando NULL
	if(input == EOF){
	...
	// Le no maximo str_size caracteres da stream, parando caso encontre uma quebra de linha, espaco ou fim de arquivo
	do{
		string[cur_size++] = (char)input;
		input = fgetc(stream);
	}while((char)input != '\n' && (char)input != ' ' && input != EOF && cur_size < str_size-1);
...
char **get_string_list(FILE *stream, int *tam, int mode){
	...
	if(scanf("%d", &size) < 1){
		size = 1;
	}else if(size >= 50) size = 49;

	if(mode == 0) length = 201;
...
int search_char(char character, char *string){
	if(string != NULL){
		int i = 0;
		while(string[i] != '\0') if(string[i++] == character) return 1;
		return 0;
	}
	fprintf(stderr, "invalid parameter to search_char()\n");
	return 0;
}
